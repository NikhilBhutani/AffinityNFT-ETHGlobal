...
describe("Exchange", () => {
  ...
  beforeEach(async () => {
    ...
    const Token = await ethers.getContractFactory("AffinityToken");
  ...
  it("is deployed", async () => {
    ...
    expect(await exchange.name()).to.equal("AffinityLP");
    expect(await exchange.symbol()).to.equal("ALP");
  ...
  describe("removeLiquidity", async () => {
    ...
    it("removes some liquidity", async () => {
      const userMaticBalanceBefore = await getBalance(owner.address);
      ...
      const userMaticBalanceAfter = await getBalance(owner.address);
      const userTokenBalanceAfter = await token.balanceOf(owner.address);

      expect(
        fromWei(userMaticBalanceAfter.sub(userMaticBalanceBefore))
    ...
    it("pays for provided liquidity", async () => {
      ...
      await exchange
        .connect(user)
        .maticToTokenSwap(toWei(18), { value: toWei(10) });
  ...
  describe("getMaticAmount", async () => {
    it("returns correct ether amount", async () => {
      await token.approve(exchange.address, toWei(2000));
      await exchange.addLiquidity(toWei(2000), { value: toWei(1000) });

      let ethOut = await exchange.getMaticAmount(toWei(2));
      expect(fromWei(ethOut)).to.equal("0.989020869339354039");

      ethOut = await exchange.getMaticAmount(toWei(100));
      expect(fromWei(ethOut)).to.equal("47.16531681753215817");

      ethOut = await exchange.getMaticAmount(toWei(2000));
      expect(fromWei(ethOut)).to.equal("497.487437185929648241");
    });
  });

  describe("maticToTokenTransfer", async () => {
    beforeEach(async () => {
      await token.approve(exchange.address, toWei(2000));
      await exchange.addLiquidity(toWei(2000), { value: toWei(1000) });
    });
    ...
    it("transfers at least min amount of tokens to recipient", async () => {
      ...
      await exchange
        .connect(user)
        .maticToTokenTransfer(toWei(1.97), user.address, { value: toWei(1) });
      ...
      const exchangeMaticBalance = await getBalance(exchange.address);
      expect(fromWei(exchangeMaticBalance)).to.equal("1001.0");
  ...
  describe("maticToTokenSwap", async () => {
    beforeEach(async () => {
      await token.approve(exchange.address, toWei(2000));
      await exchange.addLiquidity(toWei(2000), { value: toWei(1000) });
    });
    ...
    it("transfers at least min amount of tokens", async () => {
      ...
      await exchange
        .connect(user)
        .maticToTokenSwap(toWei(1.97), { value: toWei(1) });
    ...
    it("affects exchange rate", async () => {
      ...
      await exchange
        .connect(user)
        .maticToTokenSwap(toWei(9), { value: toWei(10) });
    ...
    it("fails when output amount is less than min amount", async () => {
      await expect(
        exchange.connect(user).maticToTokenSwap(toWei(2), { value: toWei(1) })
    ...
    it("allows zero swaps", async () => {
      await exchange
        .connect(user)
        .maticToTokenSwap(toWei(0), { value: toWei(0) });

      const userTokenBalance = await token.balanceOf(user.address);
      expect(fromWei(userTokenBalance)).to.equal("0.0");

      const exchangeMaticBalance = await getBalance(exchange.address);
      expect(fromWei(exchangeMaticBalance)).to.equal("1000.0");
  ...
  describe("tokenToMaticSwap", async () => {
    beforeEach(async () => {
    ...
    it("transfers at least min amount of tokens", async () => {
      ...
      await exchange.connect(user).tokenToMaticSwap(toWei(2), toWei(0.9));
    ...
    it("affects exchange rate", async () => {
      let ethOut = await exchange.getMaticAmount(toWei(20));
      expect(fromWei(ethOut)).to.equal("9.802950787206654124");

      await exchange.connect(user).tokenToMaticSwap(toWei(20), toWei(9));

      ethOut = await exchange.getMaticAmount(toWei(20));
      expect(fromWei(ethOut)).to.equal("9.61167838729939614");
    });

    it("fails when output amount is less than min amount", async () => {
      await expect(
        exchange.connect(user).tokenToMaticSwap(toWei(2), toWei(1.0))
      ).to.be.revertedWith("insufficient output amount");
    });

    it("allows zero swaps", async () => {
      const userBalanceBefore = await getBalance(user.address);
      await exchange.connect(user).tokenToMaticSwap(toWei(0), toWei(0));
